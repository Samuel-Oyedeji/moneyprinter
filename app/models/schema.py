...
class ScheduleEntryRequest(BaseModel):
    """Create one content-calendar entry."""
    ...
    post_time: str = Field(
        default="", description="Planned publish time HH:MM, informational"
    )
...
class ScheduleEntryUpdateRequest(BaseModel):
    ...
    video_count: Optional[int] = Field(default=None, ge=1, le=20)
    preset: Optional[str] = None
    post_time: Optional[str] = None
    language: Optional[str] = None
    status: Optional[str] = Field(
        default=None, description="pending | done | failed (to retry, set pending)"
    )


class ScheduleDuplicateRequest(BaseModel):
    """Copy an entry onto other dates, optionally with a new topic."""
